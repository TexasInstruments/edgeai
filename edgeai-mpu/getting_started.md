...
Edge AI accelerated microprocess from the AM6xA and TDA4x family of industrial and automotive processors. These feature the C7&trade; NPU, a neural network accelerator, which combines a SIMD-DSP and matrix multiplication accelerator for fast execution of neural nets. 
...
3) [Use TI Deep Learning (TIDL) to compile neural network models for the C7 AI accelerator and run on the target device](#ti-edge-ai-model-development-flow)
...
1) Type of AI processing, how many models, and their complexity 
    ...
    * **Note that TI Deep Learning (TIDL) software is optimized for vision-based models like CNN's and ViT's. Network architectures for language or time-series data may not be supported, including. LLM's and RNN's**
2) The number/type of sensors, e.g. cameras (and their resolution, framerate, etc.)
3) General purpose processing cores 
    *  Arm A-cores for a high level operating system and general compute
...
2) Compile models in the cloud and and run models on a cloud-host EVM with _Model Analyzer_. The version may lag behind the latest SDK and tools release
...
Follow these steps for Edge AI development: 
...
4) Bring your AI task onto the processor by compiling and running models for your target hardware.
    * Compile models yourself with [edgeai-tidl-tools](https://github.com/TexasInstruments/edgeai-tidl-tools) -- Recommended to first try with [pre-validated examples](https://github.com/TexasInstruments/edgeai-tidl-tools?tab=readme-ov-file#validate-and-benchmark-out-of-box-examples), especially Python-based examples.
    * Bring your own model (BYOM) and compile for the processor with [edgeai-tidl-tools custom model flow](https://github.com/TexasInstruments/edgeai-tidl-tools/blob/master/docs/model_compilation.md), using Python. 
...
5) Test and optimize your model for performance and accuracy
    ...
    * Ensure your as many of your model's layers are supported as possible -- [see the list of supported operators](https://github.com/TexasInstruments/edgeai-tidl-tools/blob/master/docs/operators.md).
    * Optimize accuracy with more calibration data, QAT, and hybrid quantization modes -- learn more in the [edgeai-tidl-tools docs](https://github.com/TexasInstruments/edgeai-tidl-tools/tree/master/docs) and [available compilation settings](https://github.com/TexasInstruments/edgeai-tidl-tools/blob/master/docs/model_compilation.md#compilation-options).
6) Integrate into an application with Gstreamer or TI OpenVX frameworks
